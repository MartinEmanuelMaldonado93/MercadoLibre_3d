...
import { Environment, OrbitControls, Float, Sparkles, PerformanceMonitor } from "@react-three/drei";
    ...
    <div className={css('banner3D')}>
      <div className={css('banner3D__content')}>
        <div className={css('banner3D__title')}>
          ...
          <Link className={css('banner3D__title--link')}
            style={{ textDecoration: 'none', color: 'white' }}
            to='buyproduct3d'
            onClick={()=>window.scrollTo({top:0})}>Ver más</Link>
        </div>
        ...
        <div className={css('banner3D__canvas')}>
          <Suspense fallback={<SpinnerLoading />}>
            <Canvas
              ...
              <Sparkles size={14} noise={1} color={'white'} scale={6} count={60} speed={2.4} />
              <pointLight />
              <MountainBike
                position={[-0.3, -0.8, -2]}
                rotation={[0, 10.08, 0]}
